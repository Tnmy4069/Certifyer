        ...
        <CardHeader>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              ...
              />
              <select
                className="h-10 w-full sm:w-auto rounded-lg border border-border bg-card px-3 text-sm"
                value={status}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <p className="font-semibold text-slate-900 truncate">
                            {cert.candidate?.name || "Unnamed Candidate"}
                          </p>
                          <p className="text-xs text-slate-500 truncate">{cert.candidate?.email || "—"}</p>
                          <p className="mt-1 font-mono text-xs text-slate-400">{cert.certificateNumber}</p>
                        </div>
                        <Badge variant={statusVariant(cert.status)} className="shrink-0">
                          {cert.status.replace("_", " ")}
                        </Badge>
                      </div>

                      {cert.failureReason ? (
                        <p className="text-xs text-red-600 bg-red-50 p-2 rounded-lg">{cert.failureReason}</p>
                      ) : null}

                      <div className="flex items-center justify-between text-xs text-slate-500 border-t pt-2.5">
                        <span>Downloads: <strong>{cert.downloadCount}</strong></span>
                        {cert.issuedAt ? <span>Issued: {new Date(cert.issuedAt).toLocaleDateString()}</span> : null}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1 border-t">
                        ...
                        {cert.status === "GENERATED" ? (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 text-xs gap-1 text-indigo-600 border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700 disabled:opacity-40"
